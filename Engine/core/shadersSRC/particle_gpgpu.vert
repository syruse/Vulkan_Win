...
layout(set = 0, binding = 3) uniform UBOParticleObject {
    vec4 dynamicPos;
    vec4 velocity;
    int mode; // 2: ghost particles with per-instance spawn state
} uboParticle;
...
layout (location = 9) in vec3 previousPos;
layout (location = 10) in float fading;
layout (location = 11) in float renderAlpha;
...
void main()
{
    ...
    vec3 posOrigin = inPos + inPosOrigin;
    vec3 scale = renderAlpha * mix(scaleMin, scaleMax, kFading);
    ...
    vec4 prevClip = uboViewProjection.prevViewProj * vec4(previousPos + inPosOrigin, 1.0f);
