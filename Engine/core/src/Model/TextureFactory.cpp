...
std::weak_ptr<TextureFactory::Texture> TextureFactory::createCubeTexture(const std::array<std::string_view, 6>& textureFileNames,
    ...
    if (m_textures.count(id) == 0) {
        ...
        if (Utils::VulkanCreateCubeTextureImage(m_vkState._core.getDevice(), m_vkState._core.getPhysDevice(), m_vkState._queue,
                                                m_vkState._cmdBufPool, _textureFileNames, texture->m_textureImage,
                                                texture->m_textureImageMemory) != VK_SUCCESS) {
...
std::weak_ptr<TextureFactory::Texture> TextureFactory::create2DTexture(std::string_view pTextureFileName,
    ...
    if (m_textures.count(pTextureFileName.data()) == 0) {
        ...
        if (Utils::VulkanCreateTextureImage(m_vkState._core.getDevice(), m_vkState._core.getPhysDevice(), m_vkState._queue,
                                            m_vkState._cmdBufPool, texturePath.c_str(), texture->m_textureImage,
                                            texture->m_textureImageMemory, mipLevels) != VK_SUCCESS) {
            Utils::printLog(ERROR_PARAM, "failed to create texture image ");
        }
        if (Utils::VulkanCreateImageView(m_vkState._core.getDevice(), texture->m_textureImage, VK_FORMAT_R8G8B8A8_SRGB,
                                         VK_IMAGE_ASPECT_COLOR_BIT, texture->m_textureImageView, mipLevels) != VK_SUCCESS) {
